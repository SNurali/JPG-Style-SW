          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <input
                ...
                placeholder="SmartWash Auto Shampoo 1L"
              />
            </div>
            <div>
          ...
          <div className="mb-4">
            ...
            <div className="flex flex-wrap gap-3 mb-3">
              {form.images.map((img, i) => (
                <div
                  key={i}
                  className="relative w-24 h-24 rounded-lg overflow-hidden border border-white/10 group"
                >
              ...
              <label
                className={`w-24 h-24 rounded-lg border-2 border-dashed border-white/10 flex flex-col items-center justify-center cursor-pointer hover:border-accent transition-colors ${uploadingImage ? 'opacity-50' : ''}`}
              >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-white/5">
              {loading ? (
                <tr>
                  <td colSpan={8} className="p-8 text-center text-text-muted animate-pulse">
                    Загрузка...
                  </td>
                </tr>
              ) : products.length === 0 ? (
                  ...
                  <tr key={p.id} className="hover:bg-white/[0.02]">
                    <td className="p-4">
                      {p.images?.[0] ? (
                        ...
                        <div className="w-12 h-12 rounded-lg bg-primary-light flex items-center justify-center text-lg">
                          📦
                        </div>
                      )}
                    </td>
                    ...
                    <td className="p-4 text-white">
                      ⭐ {p.rating}{' '}
                      <span className="text-text-muted text-xs">({p.reviewCount})</span>
                    </td>
                    <td className="p-4">
                      <span
                        className={`badge ${p.isActive ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}
                      >
        ...
        {pagination.totalPages > 1 && (
          <div className="flex items-center justify-between p-4 border-t border-white/5">
            <span className="text-xs text-text-muted">
              Стр. {pagination.page} из {pagination.totalPages} ({pagination.total} товаров)
            </span>
            <div className="flex gap-2">
              <button
