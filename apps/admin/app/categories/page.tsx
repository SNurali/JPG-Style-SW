...
export default function CategoriesPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editCat) {
        await api(`/api/admin/categories/${editCat.id}`, {
          method: 'PUT',
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                {form.image && (
                  <img src={`${form.image}`} alt="" className="w-12 h-12 rounded-lg object-cover" />
                )}
                <label
                  className={`btn-secondary text-xs cursor-pointer ${uploadingImage ? 'opacity-50' : ''}`}
                >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {loading ? (
          ...
          categories.map((cat) => (
            <div
              key={cat.id}
              className="glass-card p-5 hover:border-accent/30 transition-colors cursor-pointer"
            ...
            >
              <div className="flex items-start justify-between mb-3">
                {cat.image ? (
                  ...
                  <div className="w-14 h-14 rounded-lg bg-primary-light flex items-center justify-center text-2xl">
                    📁
                  </div>
                )}
                <span
                  className={`badge ${cat.isActive ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}
                >
