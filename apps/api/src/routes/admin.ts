...
const FALLBACK_ADMIN = {
  id: '1',
  email: 'admin@smartwash.uz',
  name: 'Admin',
  role: 'superadmin',
  passwordHash: '$2a$10$LqRqjw7sR5sE5q2r3j3H.OZrYp3j9T3c6d4Y5W7x8z1a0b2c3d4e',
  ...
  try {
    if (isDbAvailable()) {
      ...
      const [
        ...
        query(
          `SELECT COALESCE(SUM(total), 0) as sum FROM orders WHERE created_at >= $1 AND status != 'cancelled'`,
          [today]
        ),
      ...
      const recentOrders = await query(
        `SELECT o.*, c.first_name, c.last_name, c.phone FROM orders o JOIN customers c ON o.customer_id = c.id ORDER BY o.created_at DESC LIMIT 10`
      );
      return res.json({
        data: {
          ordersToday: parseInt(ordersToday.rows[0].count),
          revenueToday: parseInt(revenueToday.rows[0].sum),
          totalOrders: parseInt(totalOrders.rows[0].count),
          totalRevenue: parseInt(totalRevenue.rows[0].sum),
  ...
  try {
    if (isDbAvailable()) {
      ...
      const result = pending
        ? await reviewRepository.findPending(page, limit)
        : await reviewRepository.findAll(page, limit);
      return res.json(result);
    }
    res.json({ data: [], pagination: { page: 1, limit: 20, total: 0, totalPages: 0 } });
  } catch (err) {
  ...
  try {
    if (isDbAvailable()) {
      ...
      const dailyRevenue = await query(
        `SELECT DATE(created_at) as date, COUNT(*) as orders, COALESCE(SUM(total), 0) as revenue FROM orders WHERE created_at >= NOW() - INTERVAL '${days} days' AND status != 'cancelled' GROUP BY DATE(created_at) ORDER BY date ASC`
      );
    ...
    res.json({
      data: { dailyRevenue: [], topProducts: [], statusDistribution: [], paymentDistribution: [] },
    });
  } catch (err) {
...
adminRouter.post('/change-password', async (req: Request, res: Response) => {
  try {
    if (!isDbAvailable())
      return res.status(400).json({ error: 'Password change requires database' });
    const { query } = await import('../db/connection');
    ...
    const result = await query(`SELECT password_hash FROM admin_users WHERE id = $1`, [
      adminUser.id,
    ]);
